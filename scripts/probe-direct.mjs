...
async function sql(q) {
  const res = await fetch(
    "https://api.supabase.com/v1/projects/ehoytoerkxameyproivh/database/query",
    {
      method: "POST",
      headers: {
...
const a = await admin.auth.admin.createUser({
  ...
  email_confirm: true,
  user_metadata: { name: "Seller X" },
});
console.log(
  "createUser A resp:",
  JSON.stringify({ err: a.error?.message, uid: a.data?.user?.id }),
);
...
const b = await admin.auth.admin.createUser({
  ...
  password: "password123",
  email_confirm: true,
  user_metadata: { name: "Buyer X" },
});
if (b.error) {
  console.log("createUser B ERR:", b.error.message);
  process.exit(1);
}
...
const ins = await admin
  .from("listings")
  .insert({
    seller_id: a.data.user.id,
    category_id: cat.id,
    title: "Direct conv probe unit",
    description: "",
    condition: "baik",
    normal_price: 1000,
    area_label: "Bekasi",
    geom: "SRID=4326;POINT(106.9 -6.1)",
    status: "active",
  })
  .select("id,seller_id");
console.log(
  "insert listing:",
  ins.error ? `ERR ${JSON.stringify(ins.error)}` : "OK",
);
const l = ins.data;
...
const r = await anon.from("conversations").insert({
  listing_id: l.id,
  buyer_id: b.data.user.id,
  seller_id: l.seller_id,
});
console.log(
  "direct conv insert:",
  r.error ? `ERR ${r.error.code} ${r.error.message}` : "OK",
);
