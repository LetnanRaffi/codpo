...
async function makeUser(name, email) {
  ...
  const { error, data } = await sb.auth.signInWithPassword({ email, password: "password123" });
...
try {
  // ===== 0. Buat akun =====
  const A = await makeUser("Seller A", `seller.${stamp}@example.com`);
  const B = await makeUser("Buyer B", `buyer.${stamp}@example.com`);
  const C = await makeUser("Outsider C", `outsider.${stamp}@example.com`);
  cleanupUsers = [A.id, B.id, C.id];
  const apiA = api(A.token);
  const apiB = api(B.token);
  const apiC = api(C.token);
  console.log(`akun siap: A=${A.id.slice(0,8)} B=${B.id.slice(0,8)} C=${C.id.slice(0,8)}\n`);
  ...
  assert("A pasang listing BU", r.status === 201 && !!r.json?.data?.id, r.json?.error);
  const LISTING = r.json.data.id;

  r = await apiA("PATCH", `/api/listings/${LISTING}`, { normal_price: 5900000 });
  assert("A update harga listing sendiri", r.status === 200);

  r = await apiB("PATCH", `/api/listings/${LISTING}`, { normal_price: 1 });
  assert("RLS: B TIDAK bisa update listing A", r.status === 404 || r.status === 403);

  r = await apiB("DELETE", `/api/listings/${LISTING}`);
  assert("RLS: B TIDAK bisa hapus listing A", r.status === 404 || r.status === 403);

  // F. Search publik + ranking komponenal
  r = await api(null)("GET", `/api/listings?q=iPhone&lat=-6.1&lng=106.9&radius_m=50000`);
  const found = r.json?.data?.items ?? [];
  assert("search publik tanpa login menemukan listing", found.some((x) => x.id === LISTING));
  assert("skor komponenal terekspos (PRD §21)", !!found.find((x) => x.id === LISTING)?.score?.total);

  // K. Boost
  r = await apiA("POST", `/api/listings/${LISTING}/boost`, { product_code: "boost_24h" });
  assert("A beli boost listing sendiri", r.status === 200);
  r = await apiB("POST", `/api/listings/${LISTING}/boost`, { product_code: "boost_24h" });
  assert("RLS: B boost listing A ditolak", r.status >= 400);

  // ===== D. Presign + upload nyata ke R2 =====
  r = await apiA("POST", "/api/upload/presign", {
    kind: "listing", mime: "image/png", size: 1000, listing_id: LISTING,
  });
  ...
  assert("upload langsung ke R2 sukses", up.status === 200, `HTTP ${up.status}`);
  r = await apiA("POST", "/api/upload/presign", {
    kind: "listing", mime: "image/gif", size: 100, listing_id: LISTING,
  });
  assert("validasi mime: gif ditolak 415", r.status === 415);

  // ===== G. Chat =====
  r = await apiB("POST", "/api/conversations", { listing_id: LISTING });
  assert("B buka conversation listing A", r.status === 201 && !!r.json?.data?.conversation_id,
  ...
  r = await apiB("POST", `/api/conversations/${CONV}/messages`, { type: "text", body: "gan ready?" });
  assert("B kirim pesan", r.status === 201);

  r = await apiC("GET", `/api/conversations/${CONV}/messages`);
  assert("RLS: C TIDAK bisa baca chat A-B", r.status === 403 || (r.json?.data?.items ?? [1]).length === 0 ? true : false);
  // (endpoint menolak via RLS → items kosong/403)

  r = await apiC("POST", `/api/conversations/${CONV}/messages`, { type: "text", body: "iwak" });
  assert("RLS: C TIDAK bisa kirim pesan ke chat A-B", r.status >= 400);

  r = await apiA("GET", `/api/conversations/${CONV}/messages`);
  assert("A (seller) bisa baca chat", r.status === 200 && r.json.data.items.length >= 1);
  ...
  assert("B ajukan COD", r.status === 201 && !!r.json?.data?.request_id);
  ...
  assert("A accept → session+transaction dibuat", r.status === 200 && !!r.json?.data?.session_id);
  const SESS = r.json.data.session_id;

  r = await apiB("POST", `/api/cod/sessions/${SESS}/state`, { state: "completed" });
  assert("state machine: loncat accepted→completed DITOLAK", r.status >= 400);

  r = await apiB("POST", `/api/cod/sessions/${SESS}/state`, { state: "scheduled" });
  assert("transisi valid: accepted→scheduled", r.status === 200);

  // Tracking (PRD §30-31)
  r = await apiB("POST", `/api/cod/sessions/${SESS}/location`, { lat: -6.1, lng: 106.9 });
  assert("tracking sebelum sharing aktif DITOLAK", r.status >= 400);

  r = await apiB("PATCH", `/api/cod/sessions/${SESS}/location`, { enabled: true });
  ...
  r = await apiB("POST", `/api/cod/sessions/${SESS}/location`, { lat: -6.1, lng: 106.9, accuracy_m: 10 });
  assert("titik GPS pertama tersimpan", r.status === 200 && r.json?.data?.status === "stored");

  r = await apiB("POST", `/api/cod/sessions/${SESS}/location`, { lat: -6.10001, lng: 106.9 });
  assert("throttle <30s/<50m → throttled", r.json?.data?.status === "throttled");

  r = await apiA("POST", `/api/cod/sessions/${SESS}/location`, { lat: -6.11, lng: 106.9 });
  assert("GPS seller juga tersimpan", r.json?.data?.status === "stored");

  r = await apiC("GET", `/api/cod/sessions/${SESS}`);
  assert("RLS: C TIDAK bisa lihat sesi COD orang lain", r.status === 404 || r.status === 403);

  // Completion
  r = await apiA("POST", `/api/cod/sessions/${SESS}/state`, { state: "item_check" });
  assert("state machine: item_check HANYA buyer", r.status >= 400);

  r = await apiA("POST", `/api/cod/sessions/${SESS}/state`, { state: "arrived" });
  assert("transisi valid: near/otw→arrived (seller)", r.status === 200);

  r = await apiB("POST", `/api/cod/sessions/${SESS}/state`, { state: "item_check" });
  ...
  assert("listing otomatis SOLD setelah COD selesai", r.json?.data?.listing?.status === "sold");

  // ===== I. Reviews =====
  r = await apiB("POST", "/api/reviews", { transaction_id: trx.id, rating: 5, body: "mulus" });
  ...
  assert("notifikasi seller masuk (chat/COD/completed)",
    (r.json?.data?.items ?? []).length >= 3, `count=${r.json?.data?.items?.length}`);

  // ===== J. Reports + Admin =====
  r = await apiB("POST", "/api/reports", {
    target_type: "listing", target_id: LISTING, reason: "fake_item",
  });
  ...
  assert("admin bisa akses analytics", r.status === 200 && !!r.json?.data?.users);

  r = await apiC("PATCH", `/api/admin/listings/${LISTING}`, {
    action: "remove", note: "tes moderasi",
  });
  ...
  r = await apiC("PATCH", `/api/admin/users/${A.id}`, { action: "suspend", note: "tes" });
  assert("admin suspend user", r.status === 200);

  r = await apiA("POST", "/api/listings", {
    title: "suspend tidak bisa posting ya", description: "x",
    category_slug: "lainnya", condition: "baik", normal_price: 1000,
    area_label: "X", lat: -6, lng: 106,
  });
  ...
  for (const uid of cleanupUsers) await admin.auth.admin.deleteUser(uid);
}
...
if (fail) {
  console.log("Gagal:", results.filter((r) => !r.pass).map((r) => r.name).join(" | "));
  process.exit(1);
}
