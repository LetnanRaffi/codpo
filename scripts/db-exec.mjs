...
const candidates = [
  ...
  { host: `aws-0-ap-southeast-1.pooler.supabase.com`, port: 5432, user: `postgres.${ref}`, database: "postgres" },
  { host: `aws-0-ap-southeast-2.pooler.supabase.com`, port: 5432, user: `postgres.${ref}`, database: "postgres" },
  { host: `aws-0-us-east-1.pooler.supabase.com`, port: 5432, user: `postgres.${ref}`, database: "postgres" },
];
...
async function connect() {
  ...
  for (const pw of passwords) {
    for (const cfg of candidates) {
      try {
        const client = new pg.Client({ ...cfg, password: pw, ssl: { rejectUnauthorized: false }, connectionTimeoutMillis: 8000 });
...
if (mode === "push") {
  ...
  for (const f of files) {
    ...
    await client.query("insert into public._codpo_migrations(filename) values ($1)", [f]);
  }
